...
補充進度：依使用者指示，已另產出 AI 判斷視為正確的暫行標註版本 `annotation_workbook_ai_assumed_review.csv`。該版本將 AI 候選與建議轉入正式欄位，`manual_checked` 以 `ai_assumed` 標示，適合用於流程展示與初步回測，但不能等同逐案回到判決全文的嚴格人工查核。
...
AI 假設版回測已產出於 `06_交付物/is_reduced_classification_ai_assumed/`。在 2026 測試集 5 件中，Logistic Regression 的 Accuracy 為 0.400、F1-score 為 0.571、ROC-AUC 為 0.333；此結果樣本極小，僅作流程驗證，不宜作為模型有效性結論。
...
AI 假設版比例回測已產出於 `06_交付物/reduction_ratio_model_ai_assumed/`。在 2026 測試集 5 件中，Ridge Regression 的 MAE 為 0.2285、RMSE 為 0.3108、R² 為 -0.8637、分段命中率為 0.600；Lasso Regression 的 MAE 為 0.2189、RMSE 為 0.2919、R² 為 -0.6434、分段命中率為 0.600。此結果仍屬 AI 假設標註下的暫行 smoke test。
