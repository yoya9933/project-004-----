...
def render_governance_status(bundle: dict[str, Any]) -> None:
    ...
    if not release.get("approved_models"):
        st.warning(
            "目前沒有 learned model 通過 promotion gate；UI 因此自動退回 Mean baseline。"
        )
    else:
        st.success(
            "UI 只允許 baseline 與已通過 promotion gate 的模型。"
        )
...
def main() -> None:
    ...
    tab_predictions, tab_metrics, tab_gate = st.tabs(
        ["Predictions", "Backtest", "Promotion gate"]
    )
    ...
    st.info(
        "本工具只展示回測與模型治理結果，不構成法律意見，也不能取代法院或人工專業判斷。"
    )
